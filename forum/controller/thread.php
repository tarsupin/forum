		...
		<div class="post-left' . ($aviID && AVI_TYPE == "avatar" ? "-avatar" : "") . '">
			...
			<div class="post-status">
				...
				<div class="post-status-bottom">
					<div><a href="/' . $forum['url_slug'] . '/' . $threadID . '-' . $thread['url_slug'] . '?page=' . $_GET['page'] . '#p' . $post['id'] . '"><span class="icon-link"></span></a> <span title="' . date("M j, Y g:ia", $post['date_post']) . '" onclick="this.nextSibling.style.display=(this.nextSibling.style.display == \'none\' ? \'block\' : \'none\');">Posted ' . Time::fuzzy((int) $post['date_post']) . '</span><span style="display:none;">' . date("M j, Y g:ia", $post['date_post']) . '</span></div>
					<div style="margin-top:6px;"><span class="icon-clock"></span> Joined ' . date("M j, Y g:ia", $userList[$uniID]['date_joined']) . '</div>
