...
class thread_subs_by_user_schema {
	
	
/****** Plugin Variables ******/
	public $title = "Thread Subscriptions By User";		// <str> The title for this table.
	public $description = "The list of subscriptions, indexed by users (for sorting / optimization purposes).";		// <str> The description of this table.
	
	// Table Settings
	public $tableKey = "thread_subs_by_user";		// <str> The name of the table.
	public $fieldIndex = array("uni_id", "forum_id", "thread_id");	// <int:str> The field(s) used for the index (for editing, deleting, row ID, etc).
...
/****** Build the schema for the table ******/
	...
	// $schema->buildSchema();
	{
		...
		$define->set("new_posts")->description("Whether or not there have been new posts to the subscribed threads.")->pullType("select", "yes-no");
...
/****** Set the rules for interacting with this table ******/
	...
	// $schema->search($schema);	// Set the "search" options
	{
		...
		{
			case "view":
				$schema->addFields("uni_id", "forum_id", "thread_id", "new_posts");
				$schema->sort("uni_id");
			...
			case "search":
				$schema->addFields("uni_id", "forum_id", "thread_id");;
				break;
