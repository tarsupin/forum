...
class threads_stickied_schema {
	
	
/****** Plugin Variables ******/
	public $title = "Stickied Threads";		// <str> The title for this table.
	public $description = "The list of sticked threads.";		// <str> The description of this table.
	
	// Table Settings
	public $tableKey = "threads_stickied";		// <str> The name of the table.
...
/****** Build the schema for the table ******/
	...
	// $schema->buildSchema();
	{
		...
		$define->set("forum_id")->title("Forum ID")->description("The ID of the forum that the thread belongs to.")->isReadonly();
		$define->set("thread_id")->title("Thread ID")->description("The ID of the stickied thread.")->isUnique()->isReadonly();
		$define->set("sticky_level")->description("The level of the sticky.");
...
/****** Set the rules for interacting with this table ******/
	...
	// $schema->search($schema);	// Set the "search" options
	{
		...
		{
			case "view":
				$schema->addFields("forum_id", "thread_id", "sticky_level");
				$schema->sort("forum_id");
				$schema->sort("sticky_level");
